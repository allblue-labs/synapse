...
import {api, type PulseEntry, type PulseEntryStatus} from '@/lib/api';

const STATUS_TABS: {value: PulseEntryStatus | 'all'; label: string}[] = [
  {value: 'all', label: 'All'},
...
const STATUS_LABELS: Record<PulseEntryStatus, string> = {
...
const STATUS_BADGE: Record<PulseEntryStatus, React.ComponentProps<typeof Badge>['variant']> = {
  processing: 'processing',
  pending_validation: 'pending',
...
function EntryDetail({
  ...
  async function handleValidate() {
    setActing(true);
    try {
      await api.pulse.validate(entry.id, {extractedData: entry.extractedData ?? {}});
      onActionDone();
      onClose();
    } finally {
      setActing(false);
  ...
  async function handleReject() {
    setActing(true);
    try {
      await api.pulse.reject(entry.id, 'Rejected by operator');
      onActionDone();
      onClose();
    } finally {
      setActing(false);
    ...
    <Sheet open onClose={onClose} title={entry.contactName ?? entry.contactPhone} width="lg">
      <div className="space-y-6 p-6">
        ...
        {(entry.status === 'pending_validation' || entry.status === 'ready_to_confirm') && (
          <Can
            any={['pulse:validate', 'pulse:reject']}
            fallback={
...
export function QueueClient() {
  const [activeTab, setActiveTab] = useState<PulseEntryStatus | 'all'>('all');
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const result = await api.pulse.list({
